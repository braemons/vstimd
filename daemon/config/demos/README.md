...
Ordinary scene-configs, in exactly the format `scene-configs save` writes. They
...
edited and re-saved through the same `scene-configs load` / `scene-configs save` a
...
- Load a demo on a running server, change it from the overlay / web UI / a
  client, `scene-configs save` it, and copy the resulting
