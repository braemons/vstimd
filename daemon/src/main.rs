...
fn main() {
    ...
    // Advertised by the server itself, with the ports it actually bound, and
    // withdrawn when it stops (see mdns_service_advertisement.rs). Held until
    // main returns.
    #[cfg(feature = "web")]
    let advertised_web_port = args
        .web_enabled
        .unwrap_or(rig.web.enabled)
        .then(|| args.web_port.unwrap_or(rig.web.port));
    #[cfg(not(feature = "web"))]
    let advertised_web_port = None;
    let mut mdns = MdnsServiceAdvertisement::new(
        AdvertisedPorts {
            zmq_port: args.zmq_port,
            event_port: args.events_enabled.then_some(args.event_port),
            web_port: advertised_web_port,
        },
        env!("VSTIMD_VERSION"),
    );
    if args.mdns_enabled {
        ...
        log::info!("vstimd: not advertising over mDNS (--no-mdns)");
    }
...
struct Args {
    ...
    #[cfg_attr(not(feature = "web"), allow(dead_code))]
    web_port: Option<u16>,
    /// False with `--no-mdns`: a test run, or a second server on a desk, must
    /// not tell the lab it is a rig.
    mdns_enabled: bool,
    rig_config: String,
...
fn print_usage() {
    ...
    eprintln!("      --no-mdns             Do not advertise _vstimd._tcp over mDNS");
