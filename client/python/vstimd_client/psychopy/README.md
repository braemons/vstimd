# `vstimd_client.psychopy` — PsychoPy-compatible visual layer

Drop-in replacement for `psychopy.visual` that renders via `vstimd`
...
`vstimd_client.VstimdClient`. Property setters translate to calls on
...
|---|---|
| `Window` | owns `VstimdClient`; `flip()` flushes queue |
...
`vstimd_client.psychopy.visual.{Rect,Circle,Window}` against the real
