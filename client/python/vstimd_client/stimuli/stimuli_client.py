...
from vstimd_client._proto.vstimd.v1.stimuli import (
    query_pb2,
...
class StimuliClient:
    ...
    Accessed as ``conn.stimuli`` on a :class:`~vstimd_client.VstimdClient` instance.
    Groups sub-clients by stimulus family:

    * ``shapes`` — :class:`~vstimd_client.stimuli.ShapesClient`: rect, circle, ellipse, polygon
    * ``grating`` — :class:`~vstimd_client.stimuli.GratingClient`: grating stimuli
    * ``text`` — :class:`~vstimd_client.stimuli.TextClient`: text stimuli
    * ``dots`` — :class:`~vstimd_client.stimuli.DotsClient`: random dot kinematograms
    * ``shapes3d`` — :class:`~vstimd_client.stimuli.Shapes3DClient`: cubes, spheres and planes in 3-D
    * ``gaussian_splat`` — :class:`~vstimd_client.stimuli.GaussianSplatClient`: trained 3DGS scenes
