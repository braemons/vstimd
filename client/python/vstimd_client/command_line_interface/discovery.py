...
class DiscoveredServer:
    ...
    hostname: str
    """mDNS hostname, e.g. ``braemons-a1b2c3.local``."""

    addresses: tuple[str, ...] = ()
    """Resolved IP addresses, in the order reported by the backend."""

    port: int = DEFAULT_PORT
    """The SRV port: the web port (8080) since 0.3, the ZMQ port before it."""
    ...
    def zmq_port(self) -> int:
        """The ZMQ command port.

        The ``zmq_port=`` TXT record where the server publishes one. A record
        without it is from before 0.3, when the SRV port *was* the ZMQ port.
        """
        advertised = self.properties.get("zmq_port", "")
        return int(advertised) if advertised.isdigit() else self.port

    @property
    def address(self) -> str:
        """ZMQ endpoint to pass to :class:`~vstimd_client.VstimdClient`.
        ...
        return f"tcp://{host}:{self.zmq_port}"

    def _key(self) -> tuple[str, int]:
        return (self.id or self.name, self.zmq_port)
