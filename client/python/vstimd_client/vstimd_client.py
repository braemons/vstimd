...
from vstimd_client.stimuli import RectParams, ShapeAppearance, StimuliClient
from vstimd_client.system import SystemClient
from vstimd_client.vtl import VtlClient
from vstimd_client.animations import AnimationClient
from vstimd_client.conditions import ConditionsClient
from vstimd_client.scene_config import SceneConfigClient
from vstimd_client.exceptions import ProtocolError, error_for_code
...
class VstimdClient:
    ...
    * ``stimuli`` — :class:`~vstimd_client.stimuli.StimuliClient`: create and mutate stimuli
    * ``system`` — :class:`~vstimd_client.system.SystemClient`: scene-wide commands and server queries
    * ``vtl`` — :class:`~vstimd_client.VtlClient`: Virtual Trigger Line control
    * ``animations`` — :class:`~vstimd_client.AnimationClient`: frame-accurate animation sequences
    * ``scene_config`` — :class:`~vstimd_client.scene_config.SceneConfigClient`: save, load, and
      retrieve named scene-configs
    * ``conditions`` — :class:`~vstimd_client.conditions.ConditionsClient`: switch between
    ...
    def __enter__(self) -> "VstimdClient":
        return self
