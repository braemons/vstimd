"""E2E tests for 3-D stimuli (cube, sphere, plane, corridor, splats), the camera and lighting."""
...
def test_gaussian_splat_on_a_finite_track(
    ...
    splats = conn.stimuli.shapes3d.create_gaussian_splat(str(path), name="corridor")
    ...
    with pytest.raises(InvalidArgumentError, match="no such splat file"):
        conn.stimuli.shapes3d.create_gaussian_splat(str(tmp_path / "missing.splat"))
    with pytest.raises(InvalidArgumentError):
