...
def test_anim_flash_state_transitions(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_flash(s, duration_frames=30, name="flash_30", final_action_mask=FinalAction.DISABLE)
    assert conn.animations.query(a).state == AnimationState.IDLE

    conn.animations.arm(a)
    assert conn.animations.query(a).state in (AnimationState.ARMED, AnimationState.RUNNING)
...
def test_anim_flash_stimulus_visible_during_run(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_flash(s, duration_frames=60, final_action_mask=FinalAction.DISABLE)
    conn.animations.arm(a)
...
def test_anim_flash_start_trigger(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_flash(
        ...
        start_trigger=(0, 10), start_edge=VtlEdge.RISING,
        final_action_mask=FinalAction.DISABLE,
    )
    conn.animations.arm(a)

    time.sleep(0.2)
    assert conn.animations.query(a).state == AnimationState.ARMED, "should remain ARMED before trigger"
...
def test_anim_flash_disarm_resets_state(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_flash(s, duration_frames=120, start_trigger=(0, 11), start_edge=VtlEdge.RISING)
    conn.animations.arm(a)
...
def test_anim_flicker_indefinite_then_disarm(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    assert conn.animations.query(a).state == AnimationState.RUNNING, "indefinite flicker should stay RUNNING"

    conn.animations.disarm(a)
    assert conn.animations.query(a).state == AnimationState.IDLE

    info = conn.stimuli.query(s)
    assert info.anim_enabled is True, "anim_enabled should be True after disarming flicker"
...
def test_anim_flicker_off_phase_start(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_flicker(s, on_frames=30, off_frames=120, total_frames=150, start_on_phase=False)
    conn.animations.arm(a)

    _wait_for_state(conn, a, AnimationState.RUNNING, timeout=2.0)
    time.sleep(0.05)
    info = conn.stimuli.query(s)
    assert info.anim_enabled is False, "stimulus should start in off-phase (anim_enabled=False)"
    ...
    time.sleep(2.1)
    info = conn.stimuli.query(s)
    assert info.anim_enabled is True, "stimulus should be in on-phase after off-phase ends"
...
def test_anim_enable_on_trigger_edge_rising(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_enable_on_trigger_edge(
        (0, 20), s, edge=VtlEdge.RISING, enabled=True,
    )
    conn.animations.arm(a)

    time.sleep(0.1)
    assert conn.animations.query(a).state == AnimationState.RUNNING, "should be RUNNING waiting for edge"
    assert conn.stimuli.query(s).enabled is False, "stimulus must still be disabled before edge"
    ...
    assert conn.stimuli.query(s).enabled is True, "stimulus should be enabled after rising edge"
...
def test_anim_enable_on_trigger_edge_falling(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_enable_on_trigger_edge(
        (0, 21), s, edge=VtlEdge.FALLING, enabled=False,
    )
    conn.animations.arm(a)
    ...
    assert conn.stimuli.query(s).enabled is False, "stimulus should be disabled after falling edge"
...
def test_anim_couple_visibility_to_vtl_line(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_couple_visibility_to_input_trigger_line(
        (0, 30), s, polarity=True,
    )
    conn.animations.arm(a)

    _wait_for_state(conn, a, AnimationState.RUNNING, timeout=2.0)
    time.sleep(0.05)
    assert conn.stimuli.query(s).anim_enabled is False, "anim_enabled should be False when line is LOW"
    ...
    time.sleep(0.1)
    assert conn.stimuli.query(s).anim_enabled is True, "anim_enabled should be True when line is HIGH"
    ...
    time.sleep(0.1)
    assert conn.stimuli.query(s).anim_enabled is False, "anim_enabled should be False when line returns LOW"
...
def test_anim_couple_visibility_inverted_polarity(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_couple_visibility_to_input_trigger_line(
        (0, 31), s, polarity=False,
    )
    conn.animations.arm(a)

    _wait_for_state(conn, a, AnimationState.RUNNING, timeout=2.0)
    time.sleep(0.05)
    assert conn.stimuli.query(s).anim_enabled is True, "inverted polarity: line LOW → anim_enabled=True"
    ...
    time.sleep(0.1)
    assert conn.stimuli.query(s).anim_enabled is False, "inverted polarity: line HIGH → anim_enabled=False"
...
def test_anim_move_along_path_2d(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    ys = [0.0] * 41
    a = conn.animations.create_move_along_path_2d(s, x=xs, y=ys, final_action_mask=FinalAction.DISABLE)
    conn.animations.arm(a)
...
def test_anim_move_along_segments_2d(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_move_along_segments_2d(
        s, x=xs, y=ys, speed_px_per_sec=400.0, final_action_mask=FinalAction.DISABLE,
    )
    conn.animations.arm(a)
...
def test_anim_final_action_restore_state(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_flash(s, duration_frames=20, final_action_mask=FinalAction.RESTORE_STATE)
    conn.animations.arm(a)

    time.sleep(0.05)
    assert conn.stimuli.query(s).enabled is True, "stimulus should be enabled while flash is running"
    ...
    assert conn.stimuli.query(s).enabled is False, "RESTORE_STATE should restore pre-animation disabled state"
...
def test_anim_final_action_trigger_line(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    conn.vtl.set_line_name(bank=0, bit=40, direction=VtlDirection.OUTPUT, name="anim_done_out")

    s = _make_rect(conn, x=0, y=-150, enabled=False)
    a = conn.animations.create_flash(
        s, duration_frames=15,
    ...
    assert any(l.name == "anim_done_out" for l in lines), "output line should be registered"
...
def test_anim_multiple_stimuli(conn: Connection, request: pytest.FixtureRequest, step_delay: float) -> None:
    ...
    a = conn.animations.create_flash(stimuli, duration_frames=30, final_action_mask=FinalAction.DISABLE)
    conn.animations.arm(a)
    ...
    assert final == AnimationState.DONE
    for s in stimuli:
        assert conn.stimuli.query(s).enabled is False, "all three stimuli should be OFF after flash"
