...
```
vstim_server (Rust, Linux/Windows)
...
```
vstim_server/
├── build.rs                       # prost-build: compile proto/vstim.proto → OUT_DIR
├── proto/
│   └── vstim.proto
└── src/
    ...
    │   ├── file_writer.rs         # Length-prefixed protobuf file writer (.vstlog format)
    ...
    │   ├── driver.rs              # ReplayDriver: reads .vstlog, dispatches to SceneState
    │   └── log_reader.rs          # Sequential .vstlog file reader / deserialiser
    │
    ...
    │
    ├── bin/
    │   └── convert.rs             # vstim-convert: .vstlog → SQLite export tool
    │
...
- [ ] Write `build.rs`:
  ```rust
  fn main() {
      prost_build::compile_protos(&["proto/vstim.proto"], &["proto/"]).unwrap();
  }
  ```
- [ ] Write `proto/vstim.proto` (schema in §5).
- [ ] Add to `main.rs`:
  ```rust
  pub mod proto {
      include!(concat!(env!("OUT_DIR"), "/vstim.rs"));
  }
  ```
...
| Event log format | Length-prefixed protobuf records in `.vstlog` binary file | Self-describing, appendable, crash-safe, readable from Python/MATLAB via protobuf library |
| Primary log store | Flat binary `.vstlog` (always); SQLite optional and deferred | SQLite write latency is non-deterministic; binary append is safe at 240 Hz |
...
| Replay | `--replay file.vstlog` replaces ZMQ server with log reader; same `SceneState::handle_request` path | Pixel-identical frame sequence; `ShmSample` events inject logged positions into animations |
...
| `vstim-convert` | Standalone binary in workspace; `.vstlog` → SQLite | Post-hoc analysis in Python/R/SQL without touching the server |
...
Provide a thin **Python adapter library** (`vstim_compat.py`):
```python
...
- [ ] **Phase 1** — Scaffolding: `Cargo.toml`, `build.rs`, `proto/vstim.proto`, `args.rs`
...
- [ ] **Phase 13** — Event logging: `logging/` module, messenger thread, `.vstlog` file writer, ZMQ PUB publisher, `emit!` / `emit_trace!` macros; integrate `EventSender` into `SceneState` and render thread
...
- [ ] **Phase 15** — SQLite export: `sqlite_writer.rs`, deferred real-time batch writes, `vstim-convert` binary
...
`LogEvent` message written to a binary `.vstlog` file. The system has four destinations
...
**Replay** (`--replay session.vstlog`) replaces the ZMQ server and all hardware inputs
...
**SQLite export** is done post-hoc by the standalone `vstim-convert` binary. SQLite is not
...
**`vstim-convert`** is a workspace binary:
```
vstim-convert session.vstlog session.db [--min-level info]
```
