...
consistent for anyone who already uses `vstimd_client`.
...
| `vstimd-convert` | Standalone binary in workspace; `.wllog` → SQLite | Post-hoc analysis in Python/R/SQL without touching the server |
...
Provide a thin **Python adapter library** (`vstimd_compat.py`):
```python
...
**SQLite export** is done post-hoc by the standalone `vstimd-convert` binary. SQLite is not
...
**`vstimd-convert`** is a workspace binary:
```
vstimd-convert session.wllog session.db [--min-level info]
```
