...
pub fn render_frame(
    ...
    let t_tess_start = std::time::Instant::now();
    {
        ...
        let mut sc = rs.scene_renderer.scene.write().expect("scene lock poisoned");
        ...
        cache.solid.fill_meshes.retain(|h, _| *h == PHOTODIODE_HANDLE || sc.stimuli.contains_key(h));
                ...
                Stimulus::Text(text) => {
                    ...
                    for g in &glyphs {
                        let ae = rs.text.glyph_atlas.lookup(g.key).or_else(|| {
                            rs.text.glyph_atlas.insert(g.key, &g.bitmap, g.bitmap_width, g.bitmap_height)
                        });
        ...
        let geometry_changed = (pd.enabled != cache.photodiode.enabled)
            || (pd.enabled && (pd.position != cache.photodiode.position || screen_size != cache.photodiode.screen_size));
        if geometry_changed {
            let (pd_verts, pd_idxs) = tessellate_photodiode(pd, screen_size);
            cache.solid.upload(PHOTODIODE_HANDLE, &ctx.device, (&pd_verts, &pd_idxs), (&[], &[]));
            ...
            cache.solid.overwrite_fill_vertices(PHOTODIODE_HANDLE, &ctx.device, &pd_verts);
            cache.photodiode.lit = Some(pd.lit);
        }
    } // scene write lock dropped — ZMQ thread can run

    rs.text.glyph_atlas.flush(&ctx.device, ctx.graphics_queue, ctx.command_pool);
    ...
    let pipe = if wireframe { &rs.scene_renderer.wireframe_pipeline } else { &rs.scene_renderer.pipeline };
    let grate = if wireframe { &rs.scene_renderer.wireframe_grating } else { &rs.scene_renderer.grating_pipeline };

    // ── 6. Acquire swapchain image ────────────────────────────────────────────
    let t_acquire_start = std::time::Instant::now();
    let (image_index, _suboptimal) = match unsafe {
        ctx.swapchain_loader.acquire_next_image(ctx.swapchain, u64::MAX, frame.image_available, vk::Fence::null())
    } {
        Ok(r) => r,
    ...
    unsafe {
        ctx.device.reset_command_buffer(cb, vk::CommandBufferResetFlags::empty()).expect("cmd reset");
        ...
        let viewport = vk::Viewport {
            x: 0.0, y: 0.0, width: screen_w, height: screen_h, min_depth: 0.0, max_depth: 1.0,
        };
        ...
        for (h, entry) in &sc.stimuli {
            ...
            if !stim.is_visible() { continue; }

            if let Stimulus::Grating(s) = stim {
                if bound != Bound::Grating {
                    ctx.device.cmd_bind_pipeline(cb, vk::PipelineBindPoint::GRAPHICS, grate.pipeline);
                ...
                if bound != Bound::Text {
                    ctx.device.cmd_bind_pipeline(cb, vk::PipelineBindPoint::GRAPHICS, rs.text.text_pipeline.pipeline);
                    ctx.device.cmd_set_viewport(cb, 0, std::slice::from_ref(&viewport));
                    ctx.device.cmd_set_scissor(cb, 0, std::slice::from_ref(&render_area));
                    ctx.device.cmd_bind_descriptor_sets(
                        cb, vk::PipelineBindPoint::GRAPHICS, rs.text.text_pipeline.layout,
                        0, &[rs.text.glyph_atlas.descriptor_set], &[],
                    );
                    bound = Bound::Text;
                }
                ctx.device.cmd_push_constants(
                    cb, rs.text.text_pipeline.layout, vk::ShaderStageFlags::FRAGMENT, 0,
                    bytemuck::bytes_of(&TextPushConstants { color: t.params.live.color.into() }),
                );
                if let Some(mesh) = rs.scene_renderer.scene_cache.text.meshes.get(h).filter(|m| m.index_count > 0) {
                ...
                if (draw_fill || draw_stroke) && bound != Bound::Solid {
                    ctx.device.cmd_bind_pipeline(cb, vk::PipelineBindPoint::GRAPHICS, pipe.pipeline);
                    ctx.device.cmd_set_viewport(cb, 0, std::slice::from_ref(&viewport));
                    ctx.device.cmd_set_scissor(cb, 0, std::slice::from_ref(&render_area));
                    bound = Bound::Solid;
                }
                if draw_fill
                    && let Some(m) = rs.scene_renderer.scene_cache.solid.fill_meshes.get(h).filter(|m| m.index_count > 0)
                {
                ...
                if draw_stroke
                    && let Some(m) = rs.scene_renderer.scene_cache.solid.stroke_meshes.get(h).filter(|m| m.index_count > 0)
                {
                    ctx.device.cmd_bind_vertex_buffers(cb, 0, &[m.vertex_buffer], &[0]);
                    ctx.device.cmd_bind_index_buffer(cb, m.index_buffer, 0, vk::IndexType::UINT32);
                    ctx.device.cmd_draw_indexed(cb, m.index_count, 1, 0, 0, 0);
        ...
        if let Some(m) = rs.scene_renderer.scene_cache.solid.fill_meshes.get(&PHOTODIODE_HANDLE).filter(|m| m.index_count > 0) {
        ...
        if let (Some(ui), Some(store)) = (rs.ui.as_mut(), egui_store.as_ref()) {
            ui.egui_renderer.update_textures(&ctx.device, ctx.graphics_queue, ctx.command_pool, &store.textures_delta);
            ui.egui_renderer.upload_meshes(&ctx.device, &store.primitives, store.pixels_per_point);

            let egui_rp = vk::RenderPassBeginInfo::default()
                .render_pass(ctx.egui_render_pass)
                .framebuffer(ctx.framebuffers[image_index as usize])
                .render_area(render_area);
            ctx.device.cmd_begin_render_pass(cb, &egui_rp, vk::SubpassContents::INLINE);
            ctx.cmd_begin_label(cb, "egui overlay", [0.5, 0.9, 0.3, 1.0]);
            ui.egui_renderer.paint(&ctx.device, cb, &store.primitives, (ctx.extent.width, ctx.extent.height), store.pixels_per_point);
            ctx.cmd_end_label(cb);
            ctx.device.cmd_end_render_pass(cb);
        }
    ...
    unsafe { ctx.device.reset_fences(&[frame.in_flight]).expect("fence reset"); }
    ...
    match unsafe { ctx.swapchain_loader.queue_present(ctx.graphics_queue, &present_info) } {
    ...
    let tick = FrameTick {
        ...
        vblank_time,
        dropped_frames,
        phases: FramePhases { tessellate_us, fence_us, acquire_us, record_us, submit_us },
    };
