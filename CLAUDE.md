...
```bash
# Build (both backends always compiled)
cargo build --release
cargo test
cargo clippy

# Run — bare-metal DRM/Vulkan (default, requires no compositor)
cargo run --release
# or explicitly:
cargo run --release -- --mode drm

# Run — desktop/winit Vulkan (requires Wayland or X11)
cargo run --release -- --mode desktop
cargo run --release -- --mode desktop --windowed 1280x720
cargo run --release -- --mode desktop --fullscreen
...
| Module | Role |
|---|---|
| `main.rs` | Entry point: parse `--mode`, spawn ZMQ thread, run chosen backend |
...
**Desktop/winit mode (`--mode desktop`):**
- Vulkan swapchain via `VK_KHR_surface` + `ash-window` (Wayland/X11)
