...
| `proto.rs` | `include!` of prost-generated types from `OUT_DIR` (4 files in `proto/v1/`) |
...
`proto/v1/` — four files (`common.proto`, `service.proto`, `stimuli.proto`, `system.proto`); all messages are protobuf-encoded over ZMQ REQ/REP.

**Creation (system target → returns new handle):** `CreateRect`, `CreateCircle`, `CreateEllipse`

**Stimulus commands (handle > 0):** `SetEnabled`, `Delete`, `SetPosition`, `SetOrientation`, `SetFillColor`, `SetAlpha`, `SetOutlineColor`, `SetOutlineWidth`, `SetRectSize`, `SetDiscRadius`, `SetEllipseSize`, `QueryStimulus`
...
client-python/
  wonderlamp/
    ...
    stimuli/             # StimuliClient: create_rect, set_enabled, set_position, …
    system/              # SystemClient: query_server_info, set_background, …
    psychopy/            # PsychoPy compatibility layer
    exceptions.py        # error code → Python exception mapping
  examples/
    flash_rects.py       # create two rects, flash them, delete them
  tests/
    test_e2e.py          # full client/server integration tests
    test_e2e_null.py     # null renderer tests
    test_psychopy_compat.py
```
...
- egui overlay: working via custom Vulkan renderer in `render/vk/egui/`
